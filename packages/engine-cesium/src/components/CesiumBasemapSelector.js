...
const CesiumBasemapSelector = ({ onBasemapChange, currentBasemap = "none", disabled = false, }) => {
    const [selectedBasemap, setSelectedBasemap] = useState(currentBasemap);
    const { cesiumViewer, cesiumInstance } = useSceneStore();
    const handleBasemapChange = useCallback(async (basemapType) => {
            ...
            switch (basemapType) {
                case "cesium": {
                    try {
                        ...
                        cesiumViewer.imageryLayers.addImageryProvider(new cesiumInstance.IonImageryProvider({ assetId: 2 }));
                    }
                    catch (error) {
                        console.error("Error setting Cesium World Imagery:", error);
                        // Fallback to a simple URL template provider
                        try {
                            cesiumViewer.imageryLayers.addImageryProvider(new cesiumInstance.UrlTemplateImageryProvider({
                ...
                case "google-photorealistic": {
                    const cesiumKey = process.env.NEXT_PUBLIC_CESIUM_ION_KEY ||
                        process.env.NEXT_PUBLIC_CESIUM_TOKEN;
                    if (!cesiumKey) {
                        console.warn("Cesium Ion key not found. Please set NEXT_PUBLIC_CESIUM_ION_KEY environment variable.");
                    ...
                    try {
                        ...
                        const tileset = await cesiumInstance.Cesium3DTileset.fromIonAssetId(2275207);
    ...
    return (_jsxs(SettingContainer, { children: [_jsx(SettingLabel, { children: "Basemap" }), _jsx(FormControl, { fullWidth: true, size: "small", children: _jsxs(Select, { value: selectedBasemap, onChange: (e) => handleBasemapChange(e.target.value), disabled: disabled, sx: {
                        ...
                        "&.Mui-focused .MuiOutlinedInput-notchedOutline": {
                    ...
                    }, children: [_jsx(MenuItem, { value: "cesium", sx: {
                                fontSize: "0.875rem",
                                "&.Mui-selected": {
                            ...
                            }, children: "Cesium World Imagery" }), _jsx(MenuItem, { value: "google", sx: {
                                fontSize: "0.875rem",
                                "&.Mui-selected": {
                                    backgroundColor: "rgba(95, 136, 199, 0.08)",
                                    "&:hover": {
                            ...
                            }, children: "Google Satellite" }), _jsx(MenuItem, { value: "google-photorealistic", sx: {
                                fontSize: "0.875rem",
                                "&.Mui-selected": {
                                    backgroundColor: "rgba(95, 136, 199, 0.08)",
                                    "&:hover": {
                            ...
                            }, children: "Google Photorealistic" }), _jsx(MenuItem, { value: "none", sx: {
                                fontSize: "0.875rem",
                                "&.Mui-selected": {
                                    backgroundColor: "rgba(95, 136, 199, 0.08)",
                                    "&:hover": {
