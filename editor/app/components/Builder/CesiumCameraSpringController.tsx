...
const CesiumCameraSpringController: React.FC = () => {
  ...
  const startCameraAnimation = (
    ...
    const animate = (currentTime: number) => {
      ...
      // Calculate up vector parallel to the ground at current position
  ...
  useEffect(() => {
    if (
      ...
      if (currentPoint && currentPoint.position && currentPoint.target) {
        ...
        // Convert arrays to Cesium Cartesian3
        ...
        console.log(
          "[CesiumCameraSpringController] Starting camera animation to observation point:",
          {
            position,
            target,
            observationTitle: currentPoint.title,
            previewIndex,
            totalPoints: observationPoints.length,
          }
        );
      }
